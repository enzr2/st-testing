...
from .song_lists import angrysong, fearsong, happysong, sadsong
from .word_lists import angry_list, fear_list, happy_list, sad_list
...
def coursework(text):
    ...
    )  # remove punctuation text = text.split(" ") #split words into array for detection.
    text = text.split(" ")
    ...
    if total == 0:  # no emotion detected
        output.append(
            "Well, idk what you're feeling, could you maybe try and use different words to explain you're emotion? "
        )
        url = ""
    ...
    if emotion == 1:  # happy
        ...
        output.append(
            "Oh, nice! Great that you're feeling happy today \n\nbased on your feeling, try listening to {}".format(
                randoms
            )
        )
        output.append(
            "At the same time if you're feeling hungry, try making this '{}'".format(
                randomf
            )
        )
    ...
    if emotion == 2:  # sad
        ...
        randomf = random.choice(food)
        output.append(
            "Man, its alright you can get through it.\n\nwell because your feeling sad, try listening to {}".format(
                randoms
            )
        )
        output.append(
